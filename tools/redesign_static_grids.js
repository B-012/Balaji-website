...
// The regex matches the old basic grid card structure.
// We capture the image src (group 1), alt text (group 2), and the label (group 3).
const cardRegex = /<div style="position:relative; border-radius: 10px; overflow: hidden; cursor: pointer; box-shadow: 0 4px[\s\S]*?15px rgba\(0,0,0,0\.15\);" onclick="location\.href='packages\.html'">\s*<img[^>]*src="([^"]+)" alt="([^"]+)"[^>]*>\s*<div style="position:absolute; bottom:0; left:0; right:0; background:rgba\(0,0,0,0\.7\); color:#fff;\s*text-align:center; padding:10px; font-weight:800; font-size:0\.85rem; letter-spacing:2px;">([^<]+)<\/div>\s*<\/div>/g;
...
console.log('Replaced ' + count + ' basic cards with premium UX cards in packages.html');
